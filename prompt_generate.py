...
def extract_sql(sql,db_scheme,having_intersect=False,having_except=False,having_union=False):
    ...
    from_condition=parse_from_condition(from_,table_names_original,column_names_original)
    ...
    where_condition=parse_where_condition(where,table_names_original,column_names_original)
    ...
    if len(group_by_info):
        ...
        having_info=parse_having(having,table_names_original,column_names_original)
        if len(having_info):
...
def extract_sql_str(sql,db_scheme):
    #TODO 嵌套查询
    pass
...
if __name__ == '__main__':
    #sample_sql,sample_sql_path=random_sample()
    sample_sql_path='./data/spider/random_sample.sql'
    ...
    with open(sample_sql_path, 'r', encoding='utf-8') as f1 :
        ...
        for sql in sample_sql:
            print('='*50)
            promt_sql=extract_sql(sql,db_scheme=db_scheme[sql['db_id']])

            db_scheme_strs=db_scheme[sql['db_id']]['str_list']
            question=sql['question']
            prompt_db_scheme=''
            for db_str in db_scheme_strs:
                prompt_db_scheme+='# '+db_str+'\n'
            
            ans=prompt_db_scheme.join(PROMPT['OpenAI'])+question+'\n\n'+promt_sql
